...
def _json_mode_failed(exc):
    """Groq ka 400 `json_validate_failed` — model ne JSON mode mein invalid/khaali
    output diya (reasoning models par aam hai). Model theek hai, sirf JSON mode ka
    masla hai, is liye wohi model plain text mode mein dobara try karte hain."""
    low = str(exc).lower()
    return ("json_validate_failed" in low or "failed to validate json" in low
            or "json_object" in low)


def _no_json_mode(messages):
    """JSON mode ke baghair bhejne se pehle prompt mein saaf likh dete hain ke
    sirf JSON chahiye (extract_json phir markdown/prose se bhi nikaal leta hai)."""
    out = [dict(m) for m in messages]
    for m in reversed(out):
        if m.get("role") == "user":
            m["content"] = (str(m["content"]) + "\n\nOUTPUT RULE: reply with the raw "
                            "JSON object only — no markdown fences, no explanation, "
                            "no reasoning text before or after it.")
            break
    return out
...
def call_groq(messages, json_mode=False, max_tokens=CHAT_OUTPUT_TOKENS,
    ...
    JSON mode fail ho to wohi model plain text mode mein dobara chalta hai."""
    ...
    errors = []
    for model in order:
        tries = [(json_mode, messages)]
        if json_mode:                          # json mode fail ho to plain text
            tries.append((False, _no_json_mode(messages)))
        dead = False
        for use_json, msgs in tries:
            ...
            try:
                ...
                if tpm is not None:
                    ...
                    raise tpm from exc
                errors.append(f"• {model}{' (JSON mode)' if use_json else ''} → {exc}")
                if use_json and _json_mode_failed(exc):
                    continue                   # wohi model, JSON mode ke baghair
                dead = _looks_dead(exc)
                break
...
# ----------------------------------------------------------------- database
# Postgres/PostgREST column ka masla teen mukhtalif zubanon mein batate hain:
#   PGRST204 → "Could not find the 'components' column of 'secure_pc1' in the
#              schema cache"   (insert)
#   42703    → "column secure_pc1.project_title does not exist"  (select/filter)
MISSING_COL_PATTERNS = (
...
def missing_column(exc):
    """Error mein jis column ka naam hai wo wapas — warna None."""
    text = str(exc)
    for pat in MISSING_COL_PATTERNS:
...
def db_hint(exc):
    """Generic 'RLS hoga' ki jagah asal wajah ke mutabiq hidayat."""
    low = str(exc).lower()
    if "pgp_sym_encrypt" in low or "pgcrypto" in low:
        return ("`insert_secure_pc1` RPC ke andar pgcrypto ka `pgp_sym_encrypt` "
                "use hota hai magar wo extension install nahi hai. Supabase → SQL "
                "Editor mein aik dafa chalayein:\n\n"
                "```sql\ncreate extension if not exists pgcrypto with schema "
                "extensions;\n```\n\n"
                "Uske baad function ke andar `extensions.pgp_sym_encrypt(...)` likhein. "
                "Note: agar seedha table insert chal raha hai to ye RPC zaroori nahi.")
    col = missing_column(exc)
    if col:
        kind = "jsonb" if col in COMP_KEYS + DIST_KEYS else "text"
        return (f"Table `{TABLE_PROJECTS}` mein `{col}` column mojood nahi. App ne "
                "usay khud drop kar ke insert dobara try kiya, magar wo data DB mein "
                "save nahi hoga. Rakhna hai to Supabase → SQL Editor:\n\n"
                f"```sql\nalter table {TABLE_PROJECTS} add column if not exists "
                f"{col} {kind};\n```")
    if "violates row-level security" in low or "42501" in low or "permission" in low:
        return ("Row Level Security on hai magar anon key ke liye INSERT policy nahi "
                f"hai. Supabase → SQL Editor:\n\n```sql\ncreate policy \"anon insert\" "
                f"on {TABLE_PROJECTS} for insert to anon with check (true);\n```")
    if "null value in column" in low or "23502" in low:
        return ("Koi column NOT NULL hai jo app nahi bhej raha — usay nullable karein "
                "ya default de dein (error mein us ka naam mojood hai).")
    if "does not exist" in low and "relation" in low:
        return (f"Table hi nahi mila. Naam check karein (`{TABLE_PROJECTS}`) aur "
                "yaad rakhein PostgREST sirf `public` schema dekhta hai.")
    return ("Sidebar → 🔧 Diagnostics → 'Database test' se asli column naam aur row "
            "count dekhein.")
...
PAYLOAD_ALIASES = {"project_title": TITLE_KEYS, "department": DEPT_KEYS,
                   "total_budget": BUDGET_KEYS, "components": COMP_KEYS,
                   "district_allocations": DIST_KEYS,
                   "verification_status": STATUS_KEYS}


def align_payload(payload):
    """Agar table ki asli column list mil jaye to payload ke keys unhi naamon par
    map kar dete hain (camelCase table bhi chal jati hai). Jo cheez kahin fit na ho
    wo drop — us ka naam wapas aata hai taake user ko bata sakein."""
    cols, _, _ = table_columns(TABLE_PROJECTS)
    if not cols:                               # table khaali ya RLS — guess na karein
        return dict(payload), []
    index = {}
    for c in cols:
        index.setdefault(c.lower().replace("_", ""), c)
    out, dropped = {}, []
    for key, value in payload.items():
        target = None
        for cand in (key,) + tuple(PAYLOAD_ALIASES.get(key, ())):
            hit = index.get(cand.lower().replace("_", ""))
...
CORE_KEYS = ("project_title", "total_budget")   # ye drop karne se pehle RPC try karein


def _insert_dropping(body, dropped, problems, keep_core=True):
    """Jo column table mein na ho usay drop kar ke insert dobara. keep_core=True ho
    to title/budget drop karne ki bajaye ruk jate hain (pehle RPC dekh lein)."""
    while body:
        try:
            res = supabase.table(TABLE_PROJECTS).insert(body).execute()
            return _new_id(res), True
        except Exception as exc:
            problems.append(f"• table insert ({', '.join(body)}) → {exc}")
            col = missing_column(exc)
            if col is None:
                return None, False             # RLS/NOT NULL — schema ka masla nahi
            key = col if col in body else next(
                (k for k in body if k.lower().replace("_", "")
                 == col.lower().replace("_", "")), None)
            if key is None or (keep_core and key in CORE_KEYS):
                return None, False
            body.pop(key)
            dropped.append(key)
    return None, False
...
def save_project(payload, raw_text=""):
    """Pehle table insert (jo column na ho usay khud drop kar ke dobara), phir purana
    secure RPC, aur aakhir mein core column drop kar ke bhi koshish — kuch na kuch
    save ho jaye. (new_id, kaunsa rasta chala, error) wapas aata hai."""
    body, dropped = align_payload(payload)
    problems = []

    def label():
        out = "table insert"
        if dropped:
            out += (" — ye column table mein nahi hain, is liye save nahi huye: "
                    + ", ".join(f"`{d}`" for d in dropped))
        return out

    new_id, ok = _insert_dropping(body, dropped, problems)
    if ok:
        return new_id, label(), None
    ...
    new_id, ok = _insert_dropping(body, dropped, problems, keep_core=False)
    if ok:
        return new_id, label(), None
...
def parse_slice(text_slice, json_mode=True):
    msgs = [{"role": "system", "content": PARSE_SYSTEM},
            {"role": "user",
             "content": f"{PARSE_PROMPT}\n\nDocument Text:\n{text_slice}"}]
    raw = call_groq(msgs if json_mode else _no_json_mode(msgs),
                    json_mode=json_mode, max_tokens=PARSE_OUTPUT_TOKENS,
                    temperature=0.1)
...
def parse_with_shrink(text_slice):
    ...
    karte hain aur dobara bhejte hain — user ko kuch tune nahi karna parta. JSON na
    banay to aik dafa saade text mode mein bhi try karte hain."""
    current, json_mode = text_slice, True
    for attempt in range(4):
        try:
            return parse_slice(current, json_mode=json_mode)
        except TpmError as exc:
            if attempt == 3 or not (exc.limit and exc.requested):
                raise
            ...
            if attempt == 3 or not json_mode:
                raise
            json_mode = False              # JSON mode ka masla — saada text try karein
    raise RuntimeError("Shrink ke baad bhi TPM limit paar ho rahi hai.")


REVIEW_PLAIN = (
    "You are a senior auditor of the Government of Khyber Pakhtunkhwa reviewing a "
    "PC-1 development scheme. Write exactly 4 audit observations, ONE PER LINE, "
    "no numbering, no bullets, no heading, max 28 words each. Cover: total budget "
    "size, whether component costs add up to the total, district-wise spread, and "
    "what is missing or unclear. Quote the actual figures. No praise.")


def _review_items(data):
...
def _review_lines(text):
    """Plain text jawab se lines nikalna (JSON na banay to yahi rasta)."""
    out = []
    for line in str(text or "").splitlines():
        line = line.strip().lstrip("-•*").strip()
        line = re.sub(r"^\(?\d+[.)]\s*", "", line)
        if len(line) > 25 and not line.lower().startswith(("here", "observation")):
            out.append(line)
    return out[:5]
...
def ai_review(p):
    ...
    Do koshishein: JSON mode, aur wo na chale to saada text (line per observation)."""
    context = project_context(p)[:3_000]
    try:
        raw = call_groq([{"role": "system", "content": REVIEW_SYSTEM},
                         {"role": "user", "content": context}],
                        json_mode=True, max_tokens=REVIEW_OUTPUT_TOKENS,
                        temperature=0.2)
        items = _review_items(extract_json(raw))
        if items:
            return items
        first_err = ValueError("JSON mein observations khaali thay.")
    except TpmError:
        raise                                  # ye caller khud handle karta hai
    except Exception as exc:
        first_err = exc
    raw = call_groq([{"role": "system", "content": REVIEW_PLAIN},
                     {"role": "user", "content": context[:1_500]}],
                    max_tokens=380, temperature=0.2)
    items = _review_lines(raw)
    if not items:
        raise RuntimeError(f"AI review nahi ban saka. JSON koshish: {first_err} | "
                           f"text koshish ka jawab: {str(raw)[:200]}")
    return items
...
def reconcile_doc(doc, rows):
    """Teen soorat-e-haal theek karta hai: (1) insert chal gaya magar RLS ne id wapas
    nahi di — DB list mein wohi row dhoond kar us ka id apna lete hain; (2) DB row
    mein jo column nahi hai (jaise components) wo local parse se bhar dete hain;
    (3) AI review jo DB mein nahi ja saka tha, ab id milne par bhej dete hain."""
        ...
        for r in rows:
            if (str(pick(r, TITLE_KEYS, "")) == want_title
                ...
                if found is not None:
                    ...
                    doc["row"]["id"] = found
                break
    doc_id = str(doc["row"].get("id"))
    for i, r in enumerate(rows):               # DB row + local parse ko mila dein
        if str(pick(r, ("id", "uuid", "pk"))) != doc_id:
            continue
        merged, added = dict(r), False
        for key, value in doc["row"].items():
            if key in ("id", "created_at") or value in (None, "", [], {}):
                continue
            alias_hit = any(pick(r, (a,)) not in (None, "", [], {})
                            for a in (key,) + tuple(PAYLOAD_ALIASES.get(key, ())))
            if not alias_hit:
                merged[key], added = value, True
        if added:
            rows[i] = merged
        break
